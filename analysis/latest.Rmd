...
rmarkdown::render(input = here::here("analysis", "latest.Rmd"),
                  bookdown::pdf_document2(fig_caption = TRUE),
                  output_dir = here::here("output"))
# Create SI
rmarkdown::render(here::here("analysis", "supplementary.Rmd"),
                  rmarkdown::pdf_document(fig_caption = TRUE),
                  output_dir = here::here("output"))
```
...
Over the whole study period, `r modellers$n_team` independently participating forecasting teams contributed results from `r modellers$n_model` unique forecasting models (see supplementary information (SI), Table 1). We have included metadata provided by modellers in the supplement (SI Table 1), also available online [@europeancovid-19forecasthubEuropeanCovid19Forecast, @katharine_sherratt_2022_7356267]. The number of models contributing to each ensemble forecast varied over time and by forecasting target (SI Figure 1), and not all modellers created forecasts for all locations, horizons, or variables. At most, `r modellers$targets$max_scores$n_models` models contributed forecasts for `r modellers$targets$max_scores$target_variable` in `r modellers$targets$max_scores$location_name` at the `r modellers$targets$max_scores$horizon` week horizon, with an accumulated `r modellers$targets$max_scores$n_scores` forecasts for that single target over the study period. In contrast, `r modellers$targets$min_scores$target_variable` in `r modellers$targets$min_scores$location` at the `r modellers$targets$min_scores$horizon` week horizon saw the smallest number of forecasts, with only `r modellers$targets$min_scores$n_models` independent models contributing `r modellers$targets$min_scores$n_scores` forecasts at any time over the `r hub$n_weeks` week period. Similarly, not all teams forecast across all quantiles of the predictive distribution for each target. `r modellers$n_model - modellers$n_model_wis` models provided less than the full set of 23 quantiles out of the total `r modellers$n_model` models, with those forecasts without all quantiles excluded when creating the ensemble forecast. No ensemble forecast was composed of less than `r modellers$targets$min_models` independent models.
...
In relative terms, the ensemble of all models performed well compared to both its component models and the baseline. By relative WIS scaled against a baseline of 1 (where a score <1 indicates outperforming the baseline), the median score of forecasts from the Hub ensemble model  was `r modeller_scores[["score_range"]][["50%"]]`, within an interquartile range of `r modeller_scores[["score_range"]][["25%"]]` at 25% probability to `r modeller_scores[["score_range"]][["75%"]]` at 75% probability. Meanwhile the median score of forecasts across all participating models (excluding the Hub ensemble) was `r hub_scores[["score_range"]][["50%"]]` (IQR `r hub_scores[["score_range"]][["25%"]]`-`r hub_scores[["score_range"]][["75%"]]`). 
