...
In relative terms, the ensemble of all models performed well compared to both its component models and the baseline. By relative WIS scaled against a baseline of 1 (where a score <1 indicates outperforming the baseline), the median score for participating models across all submitted forecasts was `r modeller_scores[["score_range"]][["50%"]]`, within an interquartile range of `r modeller_scores[["score_range"]][["25%"]]` at 25% probability to `r modeller_scores[["score_range"]][["75%"]]` at 75% probability. Meanwhile the median score of forecasts from the ensemble model was `r hub_scores[["score_range"]][["50%"]]` (IQR `r hub_scores[["score_range"]][["25%"]]`-`r hub_scores[["score_range"]][["75%"]]`). 

Across all horizons and locations, the ensemble performed better on scaled relative WIS than `r hub_scores$wis_vs_models$Cases$p_better`% of prediction scores when forecasting cases (with a total N=`r hub_scores$wis_vs_models$Cases$n` from `hub_scores$wis_vs_models$Cases$models` unique models), and `r hub_scores$wis_vs_models$Deaths$p_better`% of prediction scores for forecasts of incident deaths (N=`r hub_scores$wis_vs_models$Deaths$n` scores from `r hub_scores$wis_vs_models$Deaths$models` models). We saw similar levels of out-performance of the ensemble when evaluating against all models including those who did not submit the full set of probabilistic quantile predictions (`r hub_scores$ae_vs_models$Cases$p_better`% for cases with N=`r hub_scores$ae_vs_models$Cases$n` scores from `r hub_scores$ae_vs_models$Cases$models` models, and `r hub_scores$ae_vs_models$Deaths$p_better`% for deaths, N=`r hub_scores$ae_vs_models$Deaths$n` scores from `r hub_scores$ae_vs_models$Deaths$models` models).
