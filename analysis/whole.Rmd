...
# See website: https://kathsherratt.github.io/euro-hub-ensemble/whole.html
...
# See rendered results: https://kathsherratt.github.io/euro-hub-ensemble/whole.html#Results
...
More detailed discussion of performance by horizon in the context of epidemic dynamics in Europe at the time: suggests possibly limited value in reporting case forecasts that go out longer than 2 weeks (though there might be some exceptions where 4-week forecasts have good performance in some countries?). There are good reasons for this: changing policy, behaviour, variants etc. Our death forecasts on the other hand outperformed the baseline at 4 weeks - could we go even further out? Or is this because deaths were fairly ?easy? with a general trend of decline (?) whereas cases had more turning points? Testing the limits of forecasting ability in this way is an important component in the investigation of forecasts and forms an important part of communicating any model-based prediction to decision makers. 

Forecast hubs have strengthened public and policy use of Covid-19 forecasts. First, open access to visualised forecasts and data is useful for academics and the public to interpret across forecasts from models with different assumptions and interpretations of current data on a like for like scale in real time. This can support perceptions and decision making, when uncertainty is particularly important to assess accurately given forecasts can both directly and indirectly influence the future of infectious disease outcomes [1,22]. Second, this is useful for creating an ensemble forecast. Our results confirm previous findings that ensembles are or are near the best performing models by error and are the most reliably consistent models in terms of appropriate coverage of uncertainty [6,23]. 

Setting a structure for collaboration can also expand the research environment for epidemic forecasting. International collaboration in particular can promote broader expertise and pooled resources, while barriers include high transaction costs of long distance and cross cultural coordination ? particularly in the pandemic context of intensified time and resource pressures [24,25]. The forecast hub framework creates a new space for sharing knowledge about methods, data, and policy. In real time, forecasters can use this knowledge to adapt forecasts to changing conditions, which may help improve forecast quality [26]. Researchers can also use the support for peer discussions to share and seek feedback on underlying methods and concepts. This is likely to be particularly useful in emergency situations, where method development may require a focus on characterising and interpreting across multiple and diverse sources of uncertainty rather than defining a single best method [22,27]. At the same time, collating time-stamped predictions at the times ensures that true out-of-sample performance of models can be tested and retrospective claims of performance avoided.

There were some limitations (particularly: data and potential bias from who did or didn?t participate). There is further scope to adapt the hub to the changing situation across Europe. We have recently extended the hub infrastructure to include short term forecasts for hospitalisations with COVID-19, although this faces additional challenges with limited data across the locations covered by the hub. It may also be of value to separately investigate models for longer term scenarios in addition to the short term forecasts, particularly as the policy focus shifts from immediate response to anticipating changes brought by vaccinations or the geographic spread of new variants [28]. 

Comment on the computational infrastructure and that this could be used elsewhere, reducing the reliance on individual models and maximising both the transparency and accuracy of model forecasts; potentially informing the response and situational awareness in future epidemics or pandemics.

Conclude by repeating the main result and that we have shown that good forecasts are possible at the short time horizon but there are clear limits to predictability especially of cases, that there is benefit from combining multiple models into an ensemble. Based on these results we have reason to expect benefits to public health from similar efforts run in close collaboration between academics and decision makers, and setting up the infrastructure for them could be an important component to future epidemic and pandemic preparedness.
