...
# See rendered: 
# https://kathsherratt.github.io/euro-hub-ensemble/whole.html
# See google doc:
...
#### Alternative ensemble methods [b]
...
We collated forecasts from multiple teams making forecasts of COVID-19 cases and deaths across countries in Europe, using an open and principled approach to standardising both forecast targets and the uncertainty around predictions. Combining these forecasts into an ensemble we found that the ensemble forecasts outperformed a baseline model at short forecast horizons.

_Interpreting results_
More detailed discussion of performance by horizon in the context of epidemic dynamics in Europe at the time: 

suggests possibly limited value in reporting case forecasts that go out longer than 2 weeks (though there might be some exceptions where 4-week forecasts have good performance in some countries?). 

There are good reasons for this: changing policy, behaviour, variants etc. 

Our death forecasts on the other hand outperformed the baseline at 4 weeks - could we go even further out? 

Or is this because deaths were fairly ?easy? with a general trend of decline (?) whereas cases had more turning points? 



_Strengths of approach_

The forecast hub infrastructure is useful for creating an ensemble forecast. Our results confirm previous findings that ensembles are or are near the best performing models by error and are the most reliably consistent models in terms of appropriate coverage of uncertainty [6,23]. 

The forecast hub infrastructure has also created wider benefits for research and its use in real time epidemic response. The hub creates a new framework for sharing knowledge about methods or data in real time, which forecasters can use to improve forecast quality by adapting models to changing conditions [26]. At the same time, collating time-stamped predictions ensures that we can test true out-of-sample performance of models and avoid retrospective claims of performance.

Further, open access to visualised forecasts and data are useful for academics and the public to interpret across forecasts from models with different assumptions and interpretations of current data on a like for like scale in real time. Uncertainty is particularly important to assess accurately given forecasts can both directly and indirectly influence the future of infectious disease outcomes [1,22]. Testing the limits of forecasting ability with these comparisons forms an important part of communicating any model-based prediction to decision makers. 

_Limits of approach and mitigation_

There were some limitations.

Our assessment of performance suffered from biases in data. Real time data can be revised over time and this would mean that the data used to create forecasts was not the same as that used to evaluate the forecast. We mitigated this by excluding some forecasts made at a time of missing, unreliable, or heavily revised data.

The result that the ensemble was among the most reliable across countries and over time could have been influenced by biases in the component forecasts. We accepted any participation . 

There is further scope to adapt the hub to the changing situation across Europe. 

We have recently extended the hub infrastructure to include short term forecasts for hospitalisations with COVID-19, although this faces additional challenges with limited data across the locations covered by the hub. 

It may also be of value to separately investigate models for longer term scenarios in addition to the short term forecasts, particularly as the policy focus shifts from immediate response to anticipating changes brought by vaccinations or the geographic spread of new variants [28]. 



_Recommendations_

Conclude by repeating the main result and that we have shown that good forecasts are possible at the short time horizon but there are clear limits to predictability especially of cases, that there is benefit from combining multiple models into an ensemble. 

We recommend using the dataset created by the European Forecast Hub to further understand real time forecast performance. 

The computational infrastructure could be used elsewhere, reducing the reliance on individual models and maximising both the transparency and accuracy of model forecasts. This can potentially inform the response and situational awareness in future epidemics or pandemics.

Based on these results we have reason to expect benefits to public health from similar efforts run in close collaboration between academics and decision makers, and setting up the infrastructure for them could be an important component to future epidemic and pandemic preparedness.

[a]expand/eqns
[b]use seb ensemble report
