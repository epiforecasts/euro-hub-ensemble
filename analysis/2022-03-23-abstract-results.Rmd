...
# Settings
here::i_am("analysis/results.Rmd")
...
source(here("code", "data-summary.R"))
```
...
_Code and data availability_ All data and code are publicly available on Github: covid19-forecast-hubeurope/euro-hub-ensemble. 
...
We scored all forecasts submitted weekly in real time over the `r hub$n_weeks` week period from `r hub$start_date` to `r hub$end_date`. Each week, forecasts were collated for incident cases and deaths, for 32 locations over the following 4 weeks, creating 256 possible forecast targets each week. 

We received `r modellers$n_model` unique forecasting models from `r modellers$n_team` independent forecasting teams. While not all modellers created forecasts for all locations, horizons, or variables, at least `r modellers$targets$min_models` independent models contributed forecasts for each location, week ahead horizon, and variable. At most, `r modellers$targets$max_scores$n_models` models contributed forecasts for `r modellers$targets$max_scores$target_variable` in `r modellers$targets$max_scores$location_name` at the `r modellers$targets$max_scores$horizon` week horizon, with an accumulated `r modellers$targets$max_scores$n_scores` forecasts for that single target over the study period. In contrast, `r modellers$targets$min_scores$target_variable` in `r modellers$targets$min_scores$location` at the `r modellers$targets$min_scores$horizon` week horizon saw the smallest number of forecasts, with only `r modellers$targets$min_scores$n_models` independent models contributing a total `r modellers$targets$min_scores$n_scores` forecasts over the `hub$n_weeks` week study period. Similarly, not all teams forecast across all quantiles of the predictive distribution for each target, with only `r modellers$n_model_wis` models providing the full set of 23 quantiles.

Together with our ensemble model (which used all available forecasts for each possible target every week), we used this dataset to create `r nrow(scores_model)` forecasting scores, where each score summarises a unique combination of model, variable, country, and week ahead horizon. All data are available: `# ADD data DOI`. 

```{r figure-1, fig.width=5, fig.height=7}
source(here("code", "figure-1.R"))
figure_1
```
