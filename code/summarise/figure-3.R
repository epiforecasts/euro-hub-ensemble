# Create figure 3: performance by location
...
# 2 week horizon (text) ---------------------------------------------------
fig3_base <- ensemble_performance_by_location(scores_model %>%
                                                filter(horizon == 2))
fig3_summary <- fig3_base$ensemble_models %>%
  mutate(across(starts_with("beat_"),
                ~ gsub(32, "all", .))) %>%
  split(.$target_variable)

figure_3 <- fig3_base$plot_location
ggsave(filename = here("output", "figures", "figure-3.png"), plot = figure_3)

fig3_cap <- "_Performance of short-term forecasts across models and
