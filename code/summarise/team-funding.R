library(dplyr)
library(stringr)
library(tidyr)

library(googlesheets4)
gs4_deauth()
authors <- read_sheet("https://docs.google.com/spreadsheets/d/19hS7r7y126J3BPBhJa20rHApFu3Hx1DQEWe7av7fX68/edit#gid=1316950565",
                      sheet = "funding")

initials <- authors %>%
  mutate(init_1 = substr(name1, 1, 1),
         init_2 = substr(name2, 1, 1),
         init_first = ifelse(!is.na(init_2), paste0(init_1, init_2), init_1),
         init_last1 = substr(name3, 1, 1),
         init_last2 = substr(name3, 2, 2),
         init_firstlast = paste0(init_first, init_last1),
         initials = ifelse((duplicated(init_firstlast) |
                              lead(duplicated(init_firstlast), 1)),
                           paste0(init_firstlast, init_last2),
                           init_firstlast),
         initials = ifelse(is.na(initials), init_firstlast, initials),
         funding_team = str_remove_all(funding_team, "\\.$")) %>%
  select(-starts_with("init_"))

funding_split <- split(initials, initials$funding_team)

paste_funding <- function(team_funding) {
  paste(paste(unlist(team_funding["initials"]),
              collapse = ", "),
        paste("funded by",
              team_funding[["funding_team"]][1]))
}

funding_statements <- purrr::map(funding_split, ~ paste_funding(.x))


# unique individual funding
individual <- filter(initials, !is.na(funding_individual))

# virginia
virginia_funding_team <- "University of Virginia"
virginia_text <- "Virginia Dept of Health Grant "
virginia_individual <- individual %>%
  filter(grepl(virginia_funding_team, funding_team)) %>%
  mutate(funding_individual = str_remove_all(funding_individual,
                                             virginia_text))
virginia_individual_split <- split(virginia_individual,
                                   virginia_individual$funding_team)
virginia_team_statement <- purrr::map(virginia_individual_split,
                                      ~ paste_funding(.x))
virginia <- paste0(", and respectively ", virginia_text, paste(virginia_individual$funding_individual, collapse = ", "))
virginia_final <- paste0(virginia_team_statement, virginia)

funding_statements[[which(grepl(virginia_funding_team, names(funding_statements)))]] <- virginia_final

# MUNI
muni_funding_team <- "MUNI"
muni <- individual %>%
  filter(grepl(muni_funding_team, funding_team))
muni_ve <- paste(muni$initials, "also supported by", muni$funding_individual,
      sep = " ")
muni_final <- paste0(funding_statements[[which(grepl(muni_funding_team, names(funding_statements)))]], "; ", muni_ve)

funding_statements[[which(grepl(muni_funding_team, names(funding_statements)))]] <- muni_final


# all together
writeLines(paste(funding_statements, collapse = ". "),
           con = here::here("output", "funding-statments.txt"))



