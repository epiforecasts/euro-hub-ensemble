...
library(dplyr)
library(ggplot2)
source(here("code", "load", "download_variant_forecasts.R"))
theme_set(theme_bw())
...
forecast_variants <- download_variant_forecasts(load_from_local = TRUE)
...
# figure plot ----------------------------------------------
fig_data_example_ensemble <- forecast_variants %>%
...
figure_example_ensemble <- fig_data_example_ensemble %>%
...
ggsave(here("output", "figures", "figure-example-ensemble.png"),
       width = 7, height = 3)



figure_example_ensemble_cap <- "_Ensemble forecasts of weekly incident cases in Germany over periods of increasing SARS-CoV-2 variants Delta (B.1.617.2, left) and Omicron (B.1.1.529, right). Black indicates observed data. Coloured ribbons represent each weekly forecast of 1-4 weeks ahead (showing median, 50%, and 90% probability). For each variant, forecasts are shown over an x-axis bounded by the earliest dates at which 5% and 99% of sequenced cases were identified as the respective variant of concern, while vertical dotted lines indicate the approximate date that the variant reached dominance (>50% sequenced cases)._"
